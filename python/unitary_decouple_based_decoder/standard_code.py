...
class StandardComplexDecoder:
    ...
    def __post_init__(self):
        ...
        product_rows, toric_rows = _parse_standard_rows(self.h_x_tilde)
        toric = (
...
def _parse_standard_rows(h_x_tilde):
    """Return product and toric row descriptors for a standard matrix.

    Args:
        h_x_tilde: Standard X-check matrix produced by decoupling.
    ...
    horizontal = one + x
    vertical = one + y
    for row in range(h_x_tilde.nrows()):
        entries = [
            (col, R(h_x_tilde[row, col]))
            for col in range(h_x_tilde.ncols())
            if h_x_tilde[row, col] != 0
        ]
